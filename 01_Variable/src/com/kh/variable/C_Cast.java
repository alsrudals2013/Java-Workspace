...
public class C_Cast {
	 ...
	 * 대입이 가능합니다. => 다른 자료형의 값을 대입하고자 한다면 "형변환"이 필수. [표현법] 자료형 변수명 = (바꿀 자료형) 값;
	 * 
	 * 2. 같은 자료형끼리만 연산 가능. => 즉, 다른자료형끼리 연산을 수행하고 싶으면 한구문은 "형변환"을 해야함. [표현법] 값 +
	 * (바꿀자료형) 값;
	 * 
	 * 형변환의 종류 1. 자동형변환 => 자동으로 형변환이 진행되기 때문에 내가 직접 형변환할 필요가 없음. => 작은바이트의 자료형 - >
	 * 큰바이트의 자료형
	 * 
	 * 2. 강제(명시적)형변환 => 자동형변환이 되지 않아 내가직접 강제로 형변환을 해야하는경우 => 큰바이트의 자료형 -> 작은바이트의 자료형
	 * 
	 * [표현법] (바꿀자료형) 값;
	 * 
	...
	public void autoCasting() {
		...
		//   float형이 실수이기때문에 long형보다 표현가능한 범위의수가 더 커서 자동형변환이 이루어짐.
		...
		/*
		 * char의 범위 0~65xxx : 각 문자마다 고유의 정수값이 정해져있따. 따라서 쌍방향 변환이
		 * 가능하다. 음수값은 오류가 난다.
		...
		byte b3 = (byte)(b1+b2);// 강제로 형변환을 해야줘야함.
	}
	...
	public void forceCasting() {
		...
		// 연산결과가 실수(double)자로형이르모 int형에 대입이 불가능함.
